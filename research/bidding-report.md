...
This is a **measured picture of a real expert bidding system**, not a textbook.
It is built by taking every recorded auction from recent World and European
team championships, dealing the real hand back onto each call, and asking: when
a world-class player made *this* bid in *this* situation, what did they actually
have? The result is an empirical range — HCP, shape, suit length and quality —
for every opening, overcall, double and response up to the four level, split by
vulnerability and by the partnership’s system where it matters.

**Why it exists.** Bridge books quote ranges like "1♥ = 11–20, five-card suit"
or "overcalls 8–16". Those are teaching approximations. This sheet replaces them
with what the best players in the world do under championship pressure, so a deal
generator can produce hands that look like the real thing — and so the ranges can
be pasted straight into the [WesDeal](./index.html) dealer as filters.

**How to read a row.** Each context (e.g. `(1♣) 1♥` = they open 1♣, we overcall
1♥) lists the actions taken and, for each, how often it was chosen (`freq`), how
many times it occurred (`n`), and the hand it showed. Key columns:

- **HCP** — high-card points as p5 / p25 / **median** / p75 / p95, so you see the
  centre and the edges of the range, not just an average.
- **bid-suit length / their suit** — the full shape distribution as per-length
  percentages (e.g. `5:66% 6:23%`); tails are lumped as `<k` / `k+`.
- **texture** — a 0–10 suit-quality index: cards A…7 weighted top-down plus a
  bonus for touching cards, normalised so a solid AKQJT suit scores 10 at any
  length (QJT98 ≈ 5.3, KQ743 ≈ 4.4, jack-high rags ≈ 1). Shown as median
  (p25–p75). This replaces raw honour-counts because top players judge a suit by
  its whole texture, not by how many top honours it contains.
- **%bal** — share of strictly balanced shapes only: 4-3-3-3, 4-4-3-2, 5-3-3-2.
  A 5-4 or 6-3-2-2 hand is *not* balanced here even though it has no short suit.
- **filter** — the same range written in the dealer’s filter language, ready to
  paste (see [Dealer integration](#dealer-integration)).

Where a bid means different things to different partnerships — a strong vs natural
1♣, transfer vs standard responses, multi vs weak 2♦ — the field is **split by
system** so the ranges aren’t blurred together; the detection is described in each
section. Generated by research/bidding/bidding.task.ts from the WBF/EBL scrape —
